...
export const ALL_PRINCIPALS: readonly Principal[] = [
  "ontology_alignment",
  "phase_boundary_discipline",
  "evidence_completeness",
  "causal_grounding",
 ...
 */
export function assertPrincipal(value: string): asserts value is Principal {
  if (!ALL_PRINCIPALS.includes(value as Principal)) {
    throw new TypeError(
      `Invalid Principal: "${value}". Must be one of: ${ALL_PRINCIPALS.join(", ")}`,
    );
  }
}
